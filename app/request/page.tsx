...
export const Page = () => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async(data) => {
    ...
    if(res.ok) {
      reset()
      toast.success('リクエストを送信しました！曲が追加されるまで少々お待ちを！', {duration: 10000})
    } else {
      toast.error('エラーが発生しました、、直接連絡していただければ対応します、、', {duration: 10000})
    }
  }
  ...
  return (
    <div className={`bg-zinc-800 min-h-screen px-4 ${
      ...
      <p className="mt-4 text-center">希望の曲などがあればこちらからお願いします！</p>
      <form className="mt-8 text-sm px-3" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          ...
          <input className="rounded-lg mt-2 w-full h-10 text-black px-2 text-lg" {...register("customerName", { required: "名前は必須です！"})} />
          <p className="h-6 text-red-400 mt-2">{errors.customerName?.message}</p>
        </div>
        <div>
          <label className="block">アーティスト名</label>
          <input className="rounded-lg mt-2 w-full h-10 text-black px-2 text-lg" {...register("artistName", { required: "アーティスト名は必須です！"})} />
          <p className="h-6 text-red-400 mt-2">{errors.artistName?.message}</p>
        </div>
        <div>
          <label className="block">曲名</label>
          <input className="rounded-lg mt-2 w-full h-10 text-black px-2 text-lg" {...register("songName", { required: "曲名は必須です！"})} />
        ...
        <input type="submit" className="mx-auto mt-8 py-3 w-full bg-rose-500 rounded-full px-2 text-lg font-semibold" value="送信" />
      </form>
